...
from agents.sub_agent import Finding, create_sub_agent_graph
...
async def decompose_node(state: OrchestratorState):
    ...
    response = await router.generate_structured(
        ...
        messages=messages,
        budget=RequestBudget(max_input_chars=get_settings_instance().planner_input_char_budget),
    ...
    messages = [
        ...
        HumanMessage(
            content=(
                ...
                "Decide whether the answer is complete. If not, provide only the most important missing questions."
            )
        ),
    ]
    response = await router.generate_structured(
...
async def synthesize_node(state: OrchestratorState):
    router = get_model_router()
    findings = state.get("findings", [])
    sources = list(dict.fromkeys(source for source in state.get("sources", []) if source))
    findings_text = "\n".join(
        [
            f"- {f.fact if hasattr(f, 'fact') else f.get('fact', 'Unknown')} "
    ...
    messages = [
        ...
        HumanMessage(
            content=(
                ...
                "Write a concise but complete Markdown report. Use only the supplied findings and end with a numbered reference list."
            )
        ),
    ]
    response = await router.generate_text(
        task_type="synthesis",
        messages=messages,
        budget=RequestBudget(max_input_chars=get_settings_instance().synthesis_input_char_budget),
        trace_id=state["thread_id"],
    )
