...
describe("extractSseEvents", () => {
  it("parses complete SSE frames and preserves trailing partial data", () => {
    const input = [
      ...
      'event: waiting_for_quota\ndata: {"thread_id":"abc","status":"waiting_for_quota","task_type":"planner","retry_after_seconds":120,"available_at":123456,"error":"quota"}\n\n',
    ...
    expect(result.events[1]).toMatchObject({ event: "waiting_for_quota", data: { task_type: "planner" } });
    expect(result.events[2]).toMatchObject({ event: "plan", data: { plan: ["A", "B"] } });
    expect(result.events[3]).toMatchObject({ event: "source_batch", data: { discovered_count: 1 } });
